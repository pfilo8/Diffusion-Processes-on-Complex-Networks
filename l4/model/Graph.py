...
class Graph:
    ...
    def addEdgeIfExists(self, u, v, weight=1):
        if u not in self.vertList.keys():
            return None
        if v not in self.vertList.keys():
            return None
    ...
    def getDegrees(self):
        return {node.id: len(node.connectedTo) for node in self.vertList.values()}
    
    def removeEdge(self, u, v):
        self.vertList[u].removeNeighbour(v)
        self.vertList[v].removeNeighbour(u)
